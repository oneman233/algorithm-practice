...
int dfs(int now,int stp) {
	if(dp[now][stp]!=inf) return dp[now][stp];
	if(stp==0) return 0;
	for(auto i:e[now])
		mmin(dp[now][stp],i.S+dfs(i.F,stp-1));
	return dp[now][stp];
}

void gao() {
	if(k&1) {
		re(i,1,n) re(j,1,m) cout<<-1<<" \n"[j==m];
		return;
	}
	memset(dp,0x3f3f,sizeof(dp));
	re(i,1,n) re(j,1,m)
		cout<<dfs(ha(i,j),k/2)*2<<" \n"[j==m];
}
...
signed main() {
	...
	re(i,1,n) re(j,1,m-1) {
		int x;
		cin>>x;
		e[ha(i,j)].pub({ha(i,j+1),x});
		e[ha(i,j+1)].pub({ha(i,j),x});
	}
	re(i,1,n-1) re(j,1,m) {
		int x;
		cin>>x;
		e[ha(i,j)].pub({ha(i+1,j),x});
		e[ha(i+1,j)].pub({ha(i,j),x});
	}
	gao();
    return 0;
}
