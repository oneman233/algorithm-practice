...
signed main() {
    ...
    cin>>_;while(_--) {
    	discre.clear();
		cin>>n;re(i,1,n) cin>>x[i];re(i,1,n) cin>>y[i],discre.pub(y[i]);
		uniq(discre);
		re(i,1,n) y[i]=lower_bound(all(discre),y[i])-discre.begin()+1;
		use.clear();
		re(i,1,n) use.pub({x[i],y[i]});
		sort(all(use));
		int ans=0;
		mem0(tr);mem0(dp);
		int flg=0,pre=-1;
		fo(i,0,use.size()) {
			if(use[i].F!=pre) {
				fo(j,flg,i) add(use[j].S,dp[j]);
				flg=i;
				pre=use[i].F;
			}
			dp[i]=sum(use[i].S-1)+1;
			mmax(ans,dp[i]);
		}
		cout<<ans<<endl;
	}
