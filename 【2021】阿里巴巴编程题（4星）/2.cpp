...
int cal(int i) { // xi+yi
	...
	while(now<i) {
		ret=(ret*ret)%mod;
		ret=(ret-2*qpow(b,now,mod)%mod+mod)%mod;
		now*=2;
	}
	return ret;
}
...
int mul(int tmp,int i,int j) { //(xi+yi)(xj+yj) 
	int ret=tmp*cal(j)%mod;
	int mx=max(i,j),mn=min(i,j);
	ret=(ret-qpow(b,min(i,j),mod)*gao(mx-mn)%mod+mod)%mod;
	return ret;
}
...
int gao(int n) {
	...
	fo(i,0,30)
		if(n&(1ll<<i)) {
			if(ans==-1) ans=cal(1ll<<i),add=(1ll<<i);
			else ans=mul(ans,add,1ll<<i),add+=(1ll<<i);
		}
	return ans;
}
