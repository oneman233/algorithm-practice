...
signed main() {
    ...
    int aa=-1,bb=-1,p=-1;
    re(i,1,n) {
		if(a[i]>aa) aa=a[i],bb=b[i],p=i;
		else if(a[i]==aa&&b[i]>bb) bb=b[i],p=i;
	}
	re(i,1,n) if(i!=p) mmax(ans,min((aa+a[i])/2.0,(bb+b[i])/2.0));
	aa=-1,bb=-1,p=-1;
	re(i,1,n) {
		if(b[i]>aa) aa=a[i],bb=b[i],p=i;
		else if(b[i]==bb&&a[i]>aa) aa=a[i],p=i;
	}
	re(i,1,n) if(i!=p) mmax(ans,min((aa+a[i])/2.0,(bb+b[i])/2.0));
	aa=-1,bb=-1,p=-1;
	re(i,1,n) {
		if(min(a[i],b[i])>min(aa,bb)) aa=a[i],bb=b[i],p=i;
		else if(min(a[i],b[i])==min(aa,bb)&&max(a[i],b[i])>max(aa,bb)) aa=a[i],bb=b[i],p=i;
	}
	re(i,1,n) if(i!=p) mmax(ans,min((aa+a[i])/2.0,(bb+b[i])/2.0));
