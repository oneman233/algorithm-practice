...
signed main() {
    FAST
	cin>>x>>y>>z>>k;
	if(k>=x+y+z-3) return cout<<x*y*z,0;
	int ans=0;
	re(i,0,min(x-1,k)) {
		int now=k-i,mn=min(y,z),mx=max(y,z);
		if(mn>=now/2+1&&mx>=now-now/2+1)
			mmax(ans,(i+1)*(now/2+1)*(now-now/2+1));
		else
			mmax(ans,(i+1)*mn*min(mx,now-(mn-1)+1));
	}
	cout<<ans;
    return 0;
}
