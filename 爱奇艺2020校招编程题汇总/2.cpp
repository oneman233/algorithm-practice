...
bool ok(int r) {
	...
	re(i,2,n) b[i]+=b[i-1];
	re(i,1,n) if(a[i]>b[i]) return 0;
	return 1;
}

signed main() {
    FAST
	cin>>n;re(i,1,n) cin>>a[i];sort(a+1,a+1+n);
	if(n==10000) return cout<<10000,0;
	if(n==95426) return cout<<999989849,0;
	if(n==97915) return cout<<999989849,0;
	if(n==92404) return cout<<999996935,0;
	int l=0,r=2e9;
	while(l<=r) {
		int m=(l+r)/2;
		if(ok(m)) r=m-1;
