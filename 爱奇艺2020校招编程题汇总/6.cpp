...
int cal(int p) {
	re(i,0,30) if((1ll<<i)>p) return (i-1)%2;
	return -1;
}

void up(int p) {
	if((n-cal(p))%2==1) tr[p]=tr[p<<1]|tr[p<<1|1];
	else tr[p]=tr[p<<1]^tr[p<<1|1];
}

void build(int p=1,int l=1,int r=(1<<n)) {
//	cout<<p<<' '<<cal(p)<<endl;
	if(l==r) {
		tr[p]=a[l];
		return;
...
void modi(int x,int v,int p=1,int l=1,int r=(1<<n)) {
	if(l==r&&l==x) {
		tr[p]=v;
		return;
	}
	int m=(l+r)/2;
	if(x<=m) modi(x,v,p<<1,l,m);
	else modi(x,v,p<<1|1,m+1,r);
	up(p);
}
