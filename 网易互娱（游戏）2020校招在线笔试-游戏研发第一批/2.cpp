...
signed main() {
    FAST
	cin>>n;re(i,1,n) cin>>a[i];cin>>q;re(i,1,q) cin>>y[i];
	re(i,1,n) sta.pub({a[i],i});
	re(i,1,q) ask.pub({y[i],i});
	sort(all(sta));reverse(all(sta));
	sort(all(ask));reverse(all(ask));
	int j=0;
	for(auto i:ask) {
		int flg=i.F,id=i.S;
		for(;j<sta.size()&&sta[j].F>flg;++j) act(sta[j].S);
		ans[id]=now;
	}
	re(i,1,q) cout<<ans[i]<<endl;
    return 0;
}
